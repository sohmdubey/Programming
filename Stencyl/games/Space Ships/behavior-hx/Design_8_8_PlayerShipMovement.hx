...
class Design_8_8_PlayerShipMovement extends ActorScript
{          	
...
if(wrapper.enabled)
{
        if(_RightButton.isMouseDown())
{
            actor.setXVelocity(_ShipSpeed);
}

        else if(_LeftButton.isMouseDown())
{
            actor.setXVelocity(-(_ShipSpeed));
}

        else if((!(_RightButton.isMouseDown()) && !(_LeftButton.isMouseDown())))
{
